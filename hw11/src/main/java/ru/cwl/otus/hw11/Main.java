...
public class Main {
    public static void main(String[] args) {
        ...
        DBService dbService = new CachedDBService(new DBServiceHibernateImpl(), cacheEngine);
        ...
        printList(userList);
        for (int i = 0; i < 10; i++) {
            UserDataSet loadedUserWithFirstId2 = dbService.load(firstId, UserDataSet.class);
            System.out.println(loadedUserWithFirstId2);
        }

        System.out.printf("hit:%d mis:%d\n", cacheEngine.getHitCount(), cacheEngine.getMisCount());
        dbService.close();
    }
